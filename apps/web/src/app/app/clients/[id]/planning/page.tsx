    ...
    <>
      ...
      <PageGrid>
        <div className="min-w-0 lg:col-span-4">
          <Card>
            <CardTitle
              kicker="Pesquisa R"
              title="Mercado"
              hint="Tavily + OpenRouter. Sem credito, nao inventamos insight."
            />
            <MarketResearchForm
              clientId={id}
        ...
        <div className="min-w-0 space-y-6 lg:col-span-8">
          <Card>
            <CardTitle kicker="Nucleo" title="Insights gravados" />
            {insightRows.length === 0 ? (
              <EmptyNote>Nenhuma pesquisa ainda. Rode a busca ao lado ou o ciclo ORBE.</EmptyNote>
            ) : (
                  ...
                  return (
                    <div key={insight.id} className="rounded-2xl border border-slate-100 p-4">
                      ...
                      <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-slate-600">
                        {(payload.contexto_mercado ?? []).slice(0, 4).map((item) => (
                          <li key={item}>{item}</li>
                        ))}
                      </ul>
                    </div>
          ...
          <Card>
            ...
            {goalRows.length === 0 ? (
              <EmptyNote>O ciclo cria as metas. Voce so ajusta titulo e notas.</EmptyNote>
            ) : (
              <div className="grid gap-3">
                {goalRows.map((goal) => (
                  <div key={goal.id} className="rounded-2xl border border-slate-100 p-4">
                    <strong className="break-words">{goal.title}</strong>
      ...
      <div className="mt-6">
        <PageGrid>
          <div className="min-w-0 lg:col-span-6">
            <Card>
              <CardTitle title="Nova meta" hint="Uso manual. O ciclo ja preenche o essencial." />
          ...
          <div className="min-w-0 lg:col-span-6">
            <Card>
              <CardTitle title="Novo indicador" hint="Deixe Plan./Real. vazios se nao houver DRE." />
