              ...
              return (
                <div key={indicator.id}>
                  <div className="flex justify-between gap-3 text-sm">
                    ...
                    <span>{PERSPECTIVE_LABELS[indicator.perspective]} · {pct(value)}</span>
                  </div>
                  <div className="mt-2 h-3 rounded-full bg-slate-100">
                    <div className="h-3 rounded-full bg-[#c8a04c]" style={{ width: `${Math.min(100, value)}%` }} />
                  </div>
                </div>
              );
            })}
